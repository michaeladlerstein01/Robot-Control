...
youbot_dh_parameters = {'a': [0 , 0.155 , 0.135 , 0 ,  0],
                        ...
                        'd' : [0.070, 0, 0, 0, 0.218],
                        'theta' : [np.pi/2, np.pi/2, 0, np.pi/2 , np.pi ]}
...
def forward_kinematics(dh_dict, joints_readings, up_to_joint=5):
    ...
    for i in range(up_to_joint):
        T1 =  standard_dh(dh_dict['a'][i] , dh_dict['alpha'][i], dh_dict['d'][i], dh_dict['theta'][i] + joints_readings[i])
        T = T @ T1
...
def fkine_wrapper(joint_msg, br):
    ...
    # your code starts here ------------------------------
    link_i = ['link_1', 'link_2' , 'link_3' , 'link_4' , 'link_5']
    for i in range(5):
        link = forward_kinematics(youbot_dh_parameters , list(joint_msg.position) , i+ 1 )
        ...
        transform.transform.translation.x = link[0 , 3]
        transform.transform.translation.y = link[1 , 3]
        transform.transform.translation.z = link[2 , 3]
        transform.transform.rotation = rotmat2q(link)
